...
sub(r"const downloadCsv = async \(csv, filename\) => \{.*?\n\};\n\nconst shareReceiptToWhatsApp", r'''const downloadCsv = async (csv, filename) => {
...
const shareReceiptToWhatsApp''', 'downloadCsv')

sub(r"const shareReceiptToWhatsApp = async \(ref, filename, captionText\) => \{.*?\n\};\n\nconst waitForPaint", r'''const shareReceiptToWhatsApp = async (ref, filename, captionText) => {
...
const waitForPaint''', 'shareReceiptToWhatsApp')

sub(r"  const exportCsv = \(rpcFn, filename\) => \{.*?\n  \};", r'''  const exportCsv = async (rpcFn, filename) => {
...
sub(r"      downloadCsv\(lines\.join\('\\n'\), `budget_bharat_full_backup_\$\{Date\.now\(\)\}\.csv`\);\n      showFeedback\('Full backup exported'\);", r"""      const exported = await downloadCsv(lines.join('\\n'), `budget_bharat_full_backup_${Date.now()}.csv`);
      showFeedback(exported ? 'Full backup exported' : 'Backup export canceled');
      return exported;""", 'full backup export')

sub(r"      setFormData\(\{\}\);\n      setEditItem\(null\);\n      setMenuView\('menu'\);\n      setIsMenuOpen\(false\);\n    \} catch \(err\) \{", r"""      const keepAddFormOpen = menuView === 'addPerson' || menuView === 'addCategory';
      setFormData({});
      setEditItem(null);
      if (!keepAddFormOpen) {
        setMenuView('menu');
...
sub(r"  const \[catType, setCatType\] = useState\('expense'\);", r"""  const [catType, setCatType] = useState(() => window.__BUDGET_BHARAT_NEW_CATEGORY_TYPE__ || 'expense');
...
sub(r"            <p className=\"text-\[10px\] font-bold text-\[#078A87\] uppercase tracking-wider mb-4\">\n              \{menuView === 'addCategory' \? `Target Ledger: \$\{catType\.toUpperCase\(\)\}` : menuView === 'addPerson' \? 'Directory Party Entry' : 'Configuration Setup'\}\n            </p>\n            <form onSubmit=\{handleFormSubmit\}", r'''            <p className="text-[10px] font-bold text-[#078A87] uppercase tracking-wider mb-4">
              {menuView === 'addCategory' ? `Target Ledger: ${catType.toUpperCase()}` : menuView === 'addPerson' ? 'Directory Party Entry' : 'Configuration Setup'}
            </p>
            ...
            <form onSubmit={handleFormSubmit}''', 'category dialog')
sub(r"onClick=\{\(\) => openAddMenu\('addCategory'\)\}", "onClick={() => openAddMenu('addCategory', type === 'INCOME' ? 'income' : 'expense')}", 'New Record category plus')
...
if 'const createPaymentReminderImage = async' not in s:
    marker = "const waitForPaint = () => new Promise(resolve => {\n  requestAnimationFrame(() => requestAnimationFrame(resolve));\n});\n"
    if marker not in s: raise SystemExit('waitForPaint marker missing')
    s = s.replace(marker, marker + helper + '\n', 1)

sub(r"  const handleSendWhatsAppReminder = \(\) => \{\n    if \(!currentLoan\) return;.*?\n  \};", r'''  const handleSendWhatsAppReminder = async () => {
    if (!currentLoan) return;
